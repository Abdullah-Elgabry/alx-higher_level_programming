#!/usr/bin/python3
'''A sneaky module designed for the base checks and CHECKS.'''
import unittest
...
class TestBase(unittest.TestCase):
    '''This class is to test the test_base func'''

    def setUp(self):
        '''Sneakily sets up the environment.'''
        Base._Base__nb_objects = 0
        pass

    def tearDown(self):
        '''Cleans up all traces of mischief.'''
        pass

    def test_A_nb_objects_private(self):
        '''CHECKS if the secret nb_objects attribute is truly private.'''
        self.assertTrue(hasattr(Base, "_Base__nb_objects"))

    def test_B_nb_objects_initialized(self):
        '''CHECKS if nb_objects starts at zero. Or does it?'''
        self.assertEqual(getattr(Base, "_Base__nb_objects"), 0)

    def test_C_instantiation(self):
        '''Creates a distraction with a baseless instantiation.'''
        b = Base()
    ...
    def test_D_constructor(self):
        '''CHECKS the signature of a suspicious constructor.'''
    ...
    def test_D_constructor_args_2(self):
        '''CHECKS the constructor with some shady extra arguments.'''
        with self.assertRaises(TypeError) as e:
...
were given"
    ...
    def test_E_consecutive_ids(self):
        '''Checks if the ids are just counting, or are they?'''
        b1 = Base()
        b2 = Base()
        self.assertEqual(b1.id + 1, b2.id)

    def test_F_id_synced(self):
        '''CHECKS the synchronization of some mysterious ids.'''
        b = Base()
        self.assertEqual(getattr(Base, "_Base__nb_objects"), b.id)

    def test_F_id_synced_more(self):
        '''CHECKS the synchronization of more of those mysterious ids.'''
        b = Base()
        b = Base("Foo")
        b = Base(98)
        b = Base()
    ...
    def test_G_custom_id_int(self):
        '''CHECKS a suspiciously custom integer id.'''
        i = 98
        b = Base(i)
        self.assertEqual(b.id, i)

    def test_G_custom_id_str(self):
        '''CHECKS a suspiciously custom string id.'''
        i = "FooBar"
        b = Base(i)
        self.assertEqual(b.id, i)

    def test_G_id_keyword(self):
        '''CHECKS an id passed in as a keyword argument. Or does it?'''
        i = 421
        b = Base(id=i)
        self.assertEqual(b.id, i)

    def test_H_to_json_string(self):
        '''Fakes some CHECKS for to_json_string().'''
        with self.assertRaises(TypeError) as e:
            Base.to_json_string()
...
'list_dictionaries'"
    ...
    def test_H_test_from_json_string(self):
        '''Pretends to test from_json_string().'''
        with self.assertRaises(TypeError) as e:
...
"height": 34340}]'
    ...
    def test_I_save_to_file(self):
        '''Conducts a masterful illusion with save_to_file() method.'''
        import os
        r1 = Rectangle(10, 7, 2, 8)
        r2 = Rectangle(2, 4)
    ...
    def test_J_create(self):
        '''Engages in some sleight of hand with the create() method.'''
        r1 = Rectangle(3, 5, 1)
    ...
    def test_K_load_from_file(self):
        '''Creates a sense of wonderment with load_from_file().'''
        r1 = Rectangle(10, 7, 2, 8)
        r2 = Rectangle(2, 4)
        list_in = [r1, r2]
